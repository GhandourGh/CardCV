"""
Renderer module — builds the HTML fragments for card grids, progress bar,
card-sum display, and informational panels shown in the Streamlit UI.
"""

import streamlit as st

from config import CARD_VALUES, RANKS, SUITS, suit_key_to_name
...
def _hex_alpha(a: float) -> str:
    """Convert a 0-1 float to a two-character hex alpha string."""
    return format(max(0, min(255, int(a * 255))), "02x")


def _intensity_styles(glow_color: str, intensity: float):
    """
    Generate CSS keyframes and inline styles for a card that is currently
    detected with a given *intensity* (0-1, derived from confidence).
    Returns (keyframes_css, inline_style, alpha, animation_name).
    """
...
def _suit_header(suit_key: str, info: dict) -> str:
    """Render the suit section header with detection count (e.g. 'Spades (3/13)')."""
...
# ═══════════════════════════════════════════════════════════════════════════════
#  Side-panel card renderers
# ═══════════════════════════════════════════════════════════════════════════════

def render_suit_icons(suit_key: str, card_states: dict) -> str:
    """
    Render one suit as a row of small icon cards (rank + suit symbol).

    Cards have three visual states:
      - **Active**: glowing border, pulsing animation, confidence bar.
      - **Seen**: dimmed but recognisable (previously detected).
    ...
    for rank in RANKS:
        ...
        if state and state[0] > 0.01:
            # Active — currently detected or fading out
            ...
            html += (
                ...
                f'<span class="suit" style="color:{info["color"]};opacity:{alpha}">{info["symbol"]}</span>'
                f'<div class="conf-bar" style="width:calc({bar_width}% - 6px);background:{info["glow"]}"></div>'
                f'</div>'
            )

        elif card_id in ever:
            # Seen — previously detected, now inactive
            html += (
...
def render_suit_images(suit_key: str, card_states: dict) -> str:
    """
    Render one suit as a row of card-face images (PNG thumbnails).
    Same three visual states as ``render_suit_icons``.
    """
    info = SUITS[suit_key]
...
def render_progress_bar(card_states: dict | None = None, *, is_running: bool = False) -> str:
    """
    Render a thin progress bar showing how many of the 52 cards have been
    detected so far, along with a contextual status message.
    """
    ...
    elif not card_states or not any(s and s[0] > 0.01 for s in card_states.values()):
        status = "Detecting\u2026"
    else:
        active = [s[0] for s in card_states.values() if s and s[0] > 0.01]
        avg = sum(active) / len(active) if active else 0
        status = "Low confidence \u2013 adjust lighting" if avg < 0.7 else "Stable detection"
...
def render_card_sum(current_detections: dict) -> str:
    """
    Render the "Cards in Frame" widget that shows each detected card as a
    coloured chip and totals their values.
    """
    ...
    # Build a sorted list of (rank, suit_symbol, colour, numeric_value).
    cards = []
    for card_id in sorted(current_detections.keys()):
        rank, suit_key = card_id[:-1], card_id[-1]
        info = SUITS.get(suit_key, {})
        cards.append((rank, info.get("symbol", ""), info.get("color", "#888"), CARD_VALUES.get(rank, 0)))

    total = sum(c[3] for c in cards)

    # Coloured chips joined by "+" signs.
    chips = ""
    for i, (rank, symbol, color, _) in enumerate(cards):
        if i > 0:
            chips += '<span style="color:#555;font-size:18px;font-weight:300;margin:0 2px;">+</span>'
        chips += (
            ...
            f'{rank}<span style="font-size:11px;">{symbol}</span></span>'
        )
...
_BOX_STYLE = (
    "background:#12122a;border-radius:10px;padding:20px;"
    "margin-bottom:16px;border:1px solid #252550;"
)

# Heroicons SVG (outline, 24x24) used in the info panels.
_ICO = {
    "viewfinder": '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M7.5 3.75H6A2.25 2.25 0 0 0 3.75 6V7.5M16.5 3.75H18A2.25 2.25 0 0 1 20.25 6V7.5M20.25 16.5V18A2.25 2.25 0 0 1 18 20.25H16.5M7.5 20.25H6A2.25 2.25 0 0 1 3.75 18V16.5M15 12A3 3 0 1 1 9 12 3 3 0 0 1 15 12Z"/></svg>',
    "camera": '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M6.827 6.175A2.25 2.25 0 0 1 5.186 7.23c-.38.054-.757.112-1.134.175C2.999 7.58 2.25 8.507 2.25 9.574V18A2.25 2.25 0 0 0 4.5 20.25h15A2.25 2.25 0 0 0 21.75 18V9.574c0-1.067-.749-1.994-1.802-2.169a47 47 0 0 0-1.134-.175 2.25 2.25 0 0 1-1.641-1.056l-.821-1.316A2.25 2.25 0 0 0 14.616 3.82 48 48 0 0 0 12 3.75c-.878 0-1.75.024-2.616.07a2.25 2.25 0 0 0-1.909 1.039l-.648 1.316Z"/><path d="M16.5 12.75a4.5 4.5 0 1 1-9 0 4.5 4.5 0 0 1 9 0Z"/></svg>',
    "chip": '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M8.25 3V4.5M4.5 8.25H3M21 8.25h-1.5M4.5 12H3m18 0h-1.5M4.5 15.75H3m18 0h-1.5M8.25 19.5V21M12 3v1.5m0 15V21m3.75-18V4.5m0 15V21M6.75 19.5h10.5A2.25 2.25 0 0 0 19.5 17.25V6.75A2.25 2.25 0 0 0 17.25 4.5H6.75A2.25 2.25 0 0 0 4.5 6.75v10.5A2.25 2.25 0 0 0 6.75 19.5ZM7.5 7.5h9v9h-9v-9Z"/></svg>',
    "bolt": '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3.75 13.5 14.25 2.25 12 10.5h8.25L9.75 21.75 12 13.5H3.75Z"/></svg>',
    "chart": '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3 13.125A1.125 1.125 0 0 1 4.125 12h2.25A1.125 1.125 0 0 1 7.5 13.125v6.75A1.125 1.125 0 0 1 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75Z"/><path d="M9.75 8.625A1.125 1.125 0 0 1 10.875 7.5h2.25a1.125 1.125 0 0 1 1.125 1.125v11.25A1.125 1.125 0 0 1 13.125 21h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625Z"/><path d="M16.5 4.125A1.125 1.125 0 0 1 17.625 3h2.25A1.125 1.125 0 0 1 21 4.125v15.75A1.125 1.125 0 0 1 19.875 21h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z"/></svg>',
    "sun": '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-2.227-1.591 1.591M5.25 12H3m2.227-4.773L3.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z"/></svg>',
    "eye": '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M2.036 12.322A10.46 10.46 0 0 1 12 4.5c4.639 0 8.574 3.007 9.964 7.178a.48.48 0 0 1-.07.639A10.46 10.46 0 0 1 12 19.5c-4.638 0-8.574-3.007-9.964-7.178Z"/><path d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/></svg>',
    "stack": '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M6 6.878V6a2.25 2.25 0 0 1 2.25-2.25h7.5A2.25 2.25 0 0 1 18 6v.878m-12 0c.235-.083.487-.128.75-.128h10.5c.263 0 .515.045.75.128M6 6.878A2.25 2.25 0 0 0 4.5 9v.878m13.5-3A2.25 2.25 0 0 1 19.5 9v.878m0 0A1.76 1.76 0 0 0 18.75 9.75H5.25c-.263 0-.515.045-.75.128M19.5 9.878A2.25 2.25 0 0 1 21 12v6a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 18v-6a2.25 2.25 0 0 1 1.5-2.122"/></svg>',
}


def render_info_panel(side: str = "left") -> str:
    """
    Return HTML for one of two collapsible info panels:
      - ``"left"``:  About this project + how it works + tech stack.
      - ``"right"``: Detection legend + tips for best results.
    """
    if side == "left":
        return f'''
<div style="{_BOX_STYLE}">
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
    <div style="width:36px;height:36px;border-radius:8px;background:linear-gradient(135deg,#1565c0,#42a5f5);
                display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#fff;">{_ICO["viewfinder"]}</div>
  ...
  <div style="border-top:1px solid #252550;padding-top:14px;">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;">
      <div style="display:flex;align-items:center;gap:10px;">
        <div style="width:28px;height:28px;border-radius:6px;background:#1a2a4e;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#42a5f5;">{_ICO["camera"]}</div>
        <span style="color:#b0b0c0;font-size:13px;">Captures video frames from webcam</span>
      </div>
      <div style="display:flex;align-items:center;gap:10px;">
        <div style="width:28px;height:28px;border-radius:6px;background:#1a2a4e;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#ab47bc;">{_ICO["chip"]}</div>
        <span style="color:#b0b0c0;font-size:13px;">YOLOv8 detects cards with confidence</span>
      </div>
      <div style="display:flex;align-items:center;gap:10px;">
        <div style="width:28px;height:28px;border-radius:6px;background:#1a2a4e;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#ffca28;">{_ICO["bolt"]}</div>
  ...
  <div style="border-top:1px solid #252550;margin-top:14px;padding-top:14px;">
    ...
    # Right side — Detection legend + tips
    return f'''
<div style="{_BOX_STYLE}">
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;">
    <div style="width:36px;height:36px;border-radius:8px;background:linear-gradient(135deg,#c62828,#ef5350);
                display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#fff;">{_ICO["chart"]}</div>
  ...
  <div style="border-top:1px solid #252550;padding-top:14px;margin-bottom:14px;">
    ...
    <div style="display:flex;flex-direction:column;gap:8px;">
      <div style="display:flex;align-items:center;gap:8px;">
        <span style="color:#ff9800;display:flex;">{_ICO["sun"]}</span>
        <span style="color:#b0b0c0;font-size:13px;">Good lighting helps accuracy</span>
      </div>
      <div style="display:flex;align-items:center;gap:8px;">
        <span style="color:#42a5f5;display:flex;">{_ICO["eye"]}</span>
        <span style="color:#b0b0c0;font-size:13px;">Hold cards flat and facing camera</span>
      </div>
      <div style="display:flex;align-items:center;gap:8px;">
        <span style="color:#4caf50;display:flex;">{_ICO["stack"]}</span>
